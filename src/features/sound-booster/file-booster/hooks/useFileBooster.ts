...
export function useFileBooster() {
  ...
  useEffect(() => {
    if (!preview) {
      ...
      setIsPlaying(false);
      // eslint-disable-next-line react-hooks/set-state-in-effect -- intentional sync to reset playback position when preview is cleared
      setCurrentTime(0);
      return;
    }
  ...
  }, [preview, activeAudition, setIsPlaying]);
