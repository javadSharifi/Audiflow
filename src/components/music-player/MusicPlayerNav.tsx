...
export function MusicPlayerNav({ activeTab, onSelectTab }: MusicPlayerNavProps): React.JSX.Element {
  ...
  return (
    ...
    >
      {/* Floating Glossy iOS Dock Container — 44dp intermediate (prev 40 → now 44, prev large 48) */}
        ...
        {PLAYER_TABS.map((tab) => {
          ...
          return (
            <button
              ...
              onClick={handleClick}
              title={label}
              className={`relative flex items-center justify-center min-h-[44px] h-11 rounded-full text-xs font-semibold cursor-pointer transition-all duration-200 active:scale-95 min-w-0 overflow-hidden ${
                isActive
                  ? "flex-none w-auto max-w-[45%] gap-2 px-4 bg-gradient-to-r from-orange-500 to-amber-500 text-white shadow-md shadow-orange-500/25 font-bold"
                  : "flex-none w-11 h-11 gap-0 px-0 sm:flex-auto sm:w-auto sm:gap-1.5 sm:px-2.5 text-zinc-600 hover:text-zinc-900 hover:bg-black/[0.04] dark:text-zinc-400 dark:hover:text-zinc-100 dark:hover:bg-white/[0.06]"
              }`}
            >
              <Icon
