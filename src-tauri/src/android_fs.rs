...
#[cfg(target_os = "android")]
pub fn call_player_set_volume_jni(volume: f32) -> Result<String, String> {
    with_jni_env(|env| {
        let cls = main_activity_class(env)?;
        let j_val = env
            .call_static_method(
                ...
                "(F)Ljava/lang/String;",
                &[jni::objects::JValue::Float(volume)],
            )
            .map_err(|e| format!("Failed to call nativePlayerSetVolume: {e}"))?;
        let j_obj = j_val.l().map_err(|e| format!("Expected object: {e}"))?;
        if j_obj.as_raw().is_null() {
            return Ok("OK".to_string());
        }
...
#[cfg(not(target_os = "android"))]
pub fn call_player_set_volume_jni(_volume: f32) -> Result<String, String> {
    Ok("OK".to_string())
}
