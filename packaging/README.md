...
- Artifact: `packaging/arch/audiflow-<version>-1-x86_64.pkg.tar.zst` (version injected from `src-tauri/tauri.conf.json` — currently **1.5.7**; `pkgrel=1`; never hardcoded).
...
- Release automation (CI producing the Arch artifact in `release.yml`) is **not part of this feature** (spec FR-010 optional; deferred). The flow is host-run only today.
